--Group by and Top N, Solution1: 2 CTEs Solution2: lag(), row_number()
with last_interact(
	select user_id, unix_timestamp as last_interaction
	from query_one
	group by user_id
	having count(unix_timestamp)>=2
	order by unix_timestamp DESC
	limit 1
),
with second_last_interact(
	select user_id, unix_timestamp as second_last_interaction
	from query_one
	where unix_timestamp < (select last_interact from last_ineract)
	group by user_id
	having count(unix_timestamp)>=1
	order by unix_timestamp DESC
	limit 1
)
select l.user_id, l.last_interaction - s.second_last_interaction as visiting_page
from last_interact l and second_last_interact s and l.user_id = s.user_id
limit 5
...
from (
		select user_id, unix_timestamp,
			   lag(unix_timestamp) over (partition by user_id order by unix_tiemstamp) as previous_time
...
--Union and Windows_Sum() -- http://www.wagonhq.com/blog/running-totals-sql
SELECT user_id, date,
